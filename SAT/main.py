...
xls_file_name = str(file.split('/')[1] ).split('.')[0] + '-' + print_heuristic[which_method] + '_final80_200.xlsx'
...
counter = 1
for sdk in range(80, 140): #len(sudokus) + 1
    print("---- {}/{} -----".format(sdk, len(sudokus)))
    og_truth_values = sudokus[sdk]
    for j in range(2,len(og_truth_values)+1):
        truth_values = set(random.sample(og_truth_values,j))
        ...
        while done == False:
            ...
            if new_clauses_count == 0:
                ...
                print("# variables {}".format(j))
                print("# splits {}".format(split_count))
                # CSV OUTPUT
                for i in range(len(results)):
                    worksheet.write(counter, i, results[i])
                counter += 1
                done = True
